...
class TestDuplicateMacRegistration(unittest.TestCase):
    def setUp(self):
    ...
    def test_duplicate_mac_registration_by_different_breeder(self, mock_send_email):
        # Scenario: Animal with MAC '00:1A:2B:3C:4D:5E' is currently registered to Breeder 3.
        # Breeder 4 attempts to register/sync this same tag.
        ...
        with app.app.test_client() as client:
            # Authenticate as Breeder 4 (Farmer_ID = 4)
            ...
            # Verify database was NOT updated (remains under Breeder 3)
            mock_animals_sheet.update.assert_not_called()
            
            # Verify Administrator was notified of the duplicate registration attempt
            recipients = [call.args[0] for call in mock_send_email.call_args_list]
            self.assertIn("sidoa065@gmail.com", recipients)
            
            # Verify the email subject contains "Duplicate"
            subjects = [call.args[1] for call in mock_send_email.call_args_list]
            self.assertTrue(any("Duplicate" in subj for subj in subjects))
            print("Test duplicate MAC registration attempt passed successfully!")

    @patch('app.send_email_async')
    def test_valid_vendu_transfer_by_different_breeder(self, mock_send_email):
        # Scenario: Animal with MAC '00:1A:2B:3C:4D:5E' is registered to Breeder 3, but status is VENDU.
        # Breeder 4 syncs this animal, transferring ownership.
        mock_animals_sheet.get_all_records.return_value = [
            {
                "ID": "1",
                "MAC": "00:1A:2B:3C:4D:5E",
                ...
                "Battery_status": "92",
                "Aler_Hist": "None",
                "Animal_status": "VENDU", # Status is VENDU
                "Farmer_ID": "3", # Belonging to Breeder 3
                "Last_Sync": ""
            }
        ]
        ...
        with app.app.test_client() as client:
            ...
            # Verify database was updated to Breeder 4
            mock_animals_sheet.update.assert_called_once()
            
            # Verify Administrator was notified of the sale
            recipients = [call.args[0] for call in mock_send_email.call_args_list]
            self.assertIn("sidoa065@gmail.com", recipients)
            
            # Verify the email subject contains "VENDU" or "Sale"
            subjects = [call.args[1] for call in mock_send_email.call_args_list]
            self.assertTrue(any("VENDU" in subj or "Sale" in subj for subj in subjects))
            print("Test valid VENDU ownership transfer passed successfully!")
